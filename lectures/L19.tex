...
Admittedly, I beg to differ from the textbook on the size of the outer joins. There's two scenarios to consider: how many matches can there be? If more than one element in $r_{2}$ can match with one (or more than one) element in $r_{1}$ then the textbook is correct. And the same for vice-versa. But what aoubt scenarios where there is (up to) one match?

We know that in a left outer join, for example, there will be $r_{1}$ tuples in the output because one tuple will appear for each tuple in $r_{1}$ whether it has a match in $r_{2}$ or not. The right outer join is symmetric. I would therefore save the headache of trying to estimate (or even reuse) an estimate of $r_{1} \bowtie r_{2}$. By this same logic the full outer join does agree with the previous paragraph.
